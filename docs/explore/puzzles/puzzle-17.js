...
export function createSVGDiagram(container) {
  ...
  svg.circle(s, cx + 95, cy + 8, 5, { fill: GOLD, stroke: RING, strokeWidth: 1.5 });
  svg.text(s, cx + 110, cy - 8, 'P1', { fontSize: 8, fill: RING });
  svg.text(s, cx + 110, cy + 12, 'P2', { fontSize: 8, fill: RING });
  ...
  const outerRing = svg.ellipse(s, cx + 140, cy, 14, 10, { stroke: RING, strokeWidth: 2.5, fill: 'none' });
  outerRing.style.transition = 'cx .12s linear, cy .12s linear';
  svg.label(s, cx + 175, cy + 20, cx + 154, cy, 'Outer ring');

  // Inner ring (inside torus) — stays trapped
  svg.ellipse(s, cx - 70, cy, 10, 7, { stroke: RING, strokeWidth: 2.5, fill: 'none' });
  svg.label(s, cx - 115, cy - 15, cx - 80, cy, 'Inner ring');

  // Motion arrows showing reroute and ring exit (toggled per step by the updater)
  const arrowReroute = svg.motionArrow(s, cx + 100, cy - 8, cx + 100, cy + 8, { label: 'Reroute cord', curvature: 0.4 });
  const arrowExit = svg.motionArrow(s, cx + 140, cy, cx + 170, cy - 20, { label: 'Pull ring free', curvature: 0.3 });
  ...
  // Key insight
  const calloutRect = svg.rect(s, 30, 390, 440, 25, { fill: WASH, stroke: RING, strokeWidth: 1, rx: 4 });
  ...
  // ---- Timeline updater: sync the 2D outer ring + badges + arrows to the solution.
  // animationSteps (0-based): 0 look, 1 identify layer, 2 reroute, 3 pull free.
  // The 3 badges map: badge1 -> steps 0/1, badge2 -> step 2, badge3 -> step 3.
  // Outer-ring keyframes mirror the 3D motion: it holds on the pattern loop, then
  // sweeps up-and-out to the right when freed.
  const restPos = [cx + 140, cy];
  const ringKeyframes = [
    [cx + 140, cy],        // 0: look — at rest on the loop
    [cx + 140, cy],        // 1: identify layer — still at rest
    [cx + 140, cy],        // 2: reroute — cord moves, ring stays
    [cx + 185, cy - 35],   // 3: pull free — sweeps up and to the right
  ];
  return function update(state) {
    if (!state) return;
    const last = ringKeyframes.length - 1;
    const i = Math.max(0, Math.min(state.stepIndex | 0, last));
    const p = Math.max(0, Math.min(state.stepProgress ?? 0, 1));

    // Move the outer ring along its 2D keyframes.
    const from = i === 0 ? restPos : ringKeyframes[i - 1];
    const to = ringKeyframes[i];
    if (outerRing && from && to) {
      outerRing.setAttribute('cx', from[0] + (to[0] - from[0]) * p);
      outerRing.setAttribute('cy', from[1] + (to[1] - from[1]) * p);
    }

    // Highlight the active step badge, dim the rest.
    ...
    if (arrowReroute) svg.highlight(arrowReroute, i === 2, { glow: false, dim: 0 });
    if (arrowExit) svg.highlight(arrowExit, i === 3, { glow: false, dim: 0 });
  };
}
