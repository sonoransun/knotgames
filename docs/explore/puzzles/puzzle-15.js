...
export function createSVGDiagram(container) {
  ...
  // Three Seifert disks (filled with semi-transparent color) — revealed during step 1
  const disk1 = svg.ellipse(s, cx, cy - 35, 25, 18, {
    fill: CORD, stroke: CORD, strokeWidth: 1.5, opacity: '0.5',
  });
  const disk2 = svg.ellipse(s, cx - 30, cy + 20, 25, 18, {
    fill: CORD, stroke: CORD, strokeWidth: 1.5, opacity: '0.5',
  });
  const disk3 = svg.ellipse(s, cx + 30, cy + 20, 25, 18, {
    fill: CORD, stroke: CORD, strokeWidth: 1.5, opacity: '0.5',
  });
  const disks = [disk1, disk2, disk3];
  disks.forEach(d => { d.style.transition = 'opacity .25s ease'; });
  ...
  // Labels
  svg.text(s, cx, cy - 55, 'Disk 1', { fontSize: 9, anchor: 'middle', fill: CORD });
  svg.text(s, cx - 55, cy + 15, 'Disk 2', { fontSize: 9, anchor: 'middle', fill: CORD });
  svg.text(s, cx + 55, cy + 15, 'Disk 3', { fontSize: 9, anchor: 'middle', fill: CORD });
  ...
  // Motion arrows showing assembly and cord push (toggled per step by the updater)
  const arrowAssemble = svg.motionArrow(s, cx - 25, cy - 70, cx, cy - 35, { label: 'Place panel 1', curvature: 0.3 });
  const arrowPush = svg.motionArrow(s, cx + 60, cy + 30, cx + 30, cy + 20, { label: 'Push cord across', curvature: 0.3 });
  const arrowFree = svg.motionArrow(s, cx + 35, cy + 5, cx + 95, cy + 5, { label: 'Pull cord free', curvature: 0.25 });
  ...
  // Key insight
  const calloutRect = svg.rect(s, 30, 375, 440, 25, { fill: WASH, stroke: RING, strokeWidth: 1, rx: 4 });
  ...
  // ---- Timeline updater: sync the 2D cord + disks + badges + arrows to the solution.
  // Steps (0-based): 0 = look (linked), 1 = insert panels, 2 = push cord across, 3 = pull free.
  // The cord stays at rest over the surface, then slides off to the right to escape.
  const cordRest = [cx, cy + 5];
  const cordPos = [
    [cx, cy + 5],         // step 0: linked, at rest
    [cx, cy + 5],         // step 1: still linked while panels fill in
    [cx + 50, cy + 5],    // step 2: pushed sideways across the surface
    [cx + 110, cy + 5],   // step 3: pulled clear of the knot
  ];
  return function update(state) {
    const n = cordPos.length;
    const i = Math.max(0, Math.min(state.stepIndex ?? 0, n - 1));
    const p = Math.max(0, Math.min(state.stepProgress ?? 0, 1));
    const from = i === 0 ? cordRest : cordPos[i - 1];
    const to = cordPos[i];
    if (cord) {
      cord.setAttribute('cx', from[0] + (to[0] - from[0]) * p);
      cord.setAttribute('cy', from[1] + (to[1] - from[1]) * p);
      cord.style.opacity = i >= 3 ? String(1 - 0.5 * p) : '1';
    }

    // Disks fade in as the panels are inserted (step 1), stay solid after.
    const diskOpacity = i === 0 ? 0.5 * p : 0.5;
    disks.forEach(d => { if (d) d.style.opacity = String(diskOpacity); });

    // Badges: step 0/1 -> insert, step 2 -> push, step 3 -> pull free.
    ...
    svg.highlight(arrowAssemble, i <= 1, { glow: false, dim: 0 });
    svg.highlight(arrowPush, i === 2, { glow: false, dim: 0 });
