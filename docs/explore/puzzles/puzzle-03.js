...
export function createSVGDiagram(container) {
  ...
  // Cord: back side (through top window, dashed) — animatable; the bight that
  // gets pulled over the bar end as the puzzle is solved.
  const cordBack = svg.path(s, `M ${fx + 20} ${crossY - 20} L ${fx + 20} ${crossY - 40} Q 250 ${crossY - 60} ${fx + fw - 20} ${crossY - 40} L ${fx + fw - 20} ${crossY - 20}`, {
    stroke: CORD, strokeWidth: 2, fill: 'none', dashArray: '5,4',
  });
  const cordBackD0 = `M ${fx + 20} ${crossY - 20} L ${fx + 20} ${crossY - 40} Q 250 ${crossY - 60} ${fx + fw - 20} ${crossY - 40} L ${fx + fw - 20} ${crossY - 20}`;
  // Solved bight: pulled off past the right end of the crossbar (no longer wraps it).
  const cordBackD1 = `M ${fx + 20} ${crossY - 20} L ${fx + 20} ${crossY - 30} Q ${fx + fw + 5} ${crossY - 50} ${fx + fw + 10} ${crossY - 20} L ${fx + fw - 20} ${crossY - 20}`;
  ...
  // Crossing indicators
  svg.text(s, fx + 15, crossY - 5, 'twist A', { fontSize: 9, fill: NEG, fontWeight: 'bold' });
  svg.text(s, fx + fw - 35, crossY - 5, 'twist B', { fontSize: 9, fill: CORD, fontWeight: 'bold' });
  ...
  const arrowPull = svg.motionArrow(s, fx + fw - 20, crossY - 20, fx + fw + 15, crossY - 10, { label: 'Pull over end', curvature: 0.3 });
  const arrowSlide = svg.motionArrow(s, 270, crossY + 65, 320, crossY + 80, { label: 'Slide ring off', curvature: 0.3 });
  ...
  const badge1 = svg.stepBadge(s, fx + fw + 25, crossY - 30, 1, 2);
  svg.actionLabel(s, fx + fw + 25, crossY - 17, 'Pull cord over bar end');
  const badge2 = svg.stepBadge(s, 320, crossY + 90, 2, 2);
  svg.actionLabel(s, 320, crossY + 103, 'Slide ring off');
  const badges = [badge1, badge2];

  // Key insight
  const calloutRect = svg.rect(s, 30, 355, 440, 35, { fill: 'var(--dia-wash, #ece3d0)', stroke: 'var(--dia-ring, #b97d12)', strokeWidth: 1, rx: 4 });
  ...
  // ---- Timeline updater: sync the 2D cord bight + ring + badges + arrows
  // Ring keyframes: rests below the bar (steps 0-1), then slides off to the
  // lower-right on the final step, mirroring the 3D ring's exit direction.
  const ringRest = [250, crossY + 65];
  const ringEnd = [340, crossY + 110];
  return function update(state) {
    const i = Math.max(0, Math.min(state.stepIndex | 0, 2));
    const p = state.stepProgress ?? 0;

    // Cord bight pulls over the right bar end during step 1 (the "unhitch").
    const pull = i >= 2 ? 1 : i === 1 ? p : 0;
    cordBack.setAttribute('d', pull > 0.5 ? cordBackD1 : cordBackD0);

    // Ring slides off only on the final step.
    const slide = i >= 2 ? p : 0;
    ring.setAttribute('cx', ringRest[0] + (ringEnd[0] - ringRest[0]) * slide);
    ring.setAttribute('cy', ringRest[1] + (ringEnd[1] - ringRest[1]) * slide);

    // Badges: step 1 (pull) highlights badge 0, step 2 (slide) highlights badge 1.
    const phase = i <= 1 ? 0 : 1;
    badges.forEach((b, k) => svg.highlight(b, k === phase, { dim: 0.3, color: CORD }));

    // Motion arrows follow the active manipulation.
    svg.highlight(arrowPull, i === 1, { glow: false, dim: 0 });
    svg.highlight(arrowSlide, i >= 2, { glow: false, dim: 0 });
  };
}
