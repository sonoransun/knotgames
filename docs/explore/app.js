...
let svgUpdater = null; // optional per-puzzle 2D-diagram updater (timeline sync)
...
async function loadPuzzle(id) {
  ...
  // Create SVG diagram. createSVGDiagram may optionally return a timeline
  // updater — a function (state) or an object with .update(state) — so the 2D
  // panel can animate in lockstep with the solution + scrubber.
  if (puzzleModule.createSVGDiagram) {
    const ret = puzzleModule.createSVGDiagram(svgContainer);
    svgUpdater = typeof ret === 'function' ? ret
      : (ret && typeof ret.update === 'function' ? ret.update.bind(ret) : null);
  }
    ...
    try {
      animController = new AnimationController(
        ...
        (state) => {
          ...
          if (svgUpdater) svgUpdater(state);
        }
      );
      ...
      // Prime the 2D diagram to step 0 so it matches the initial 3D state.
      if (svgUpdater) svgUpdater({ stepIndex: 0, stepProgress: 0, step: puzzleModule.animationSteps[0], totalProgress: 0 });
    } catch (err) {
...
// Load puzzle from hash if present
const hashId = parseInt(location.hash.replace('#', ''));
if (hashId >= 1 && hashId <= puzzleRegistry.length) {
  loadPuzzle(hashId);
}
