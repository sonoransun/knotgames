...
import { easeInOut, easeIn, easeOut, linear } from './easing.js';
import * as logger from './logger.js';

// per-step easing can be named in animationSteps (`easing: 'easeOut'`) or a fn
...
export class AnimationController {
  ...
  _emitUpdate() {
    ...
    const stepEasing = this.steps[idx]?.easing;
    const ease = typeof stepEasing === 'function' ? stepEasing : (EASINGS[stepEasing] || easeInOut);
    const easedProgress = ease(stepProgress);
...
// Helper: gimbal-free interpolation between two Euler rotations (radians,
// [x,y,z]) via quaternion slerp. Returns an Euler array. Use this instead of
// lerping Euler angles directly for coupled/large rotations.
const _qa = new THREE.Quaternion();
const _qb = new THREE.Quaternion();
const _eu = new THREE.Euler();
export function slerpEuler(from, to, t) {
  _qa.setFromEuler(_eu.set(from[0], from[1], from[2]));
  _qb.setFromEuler(_eu.set(to[0], to[1], to[2]));
  _qa.slerp(_qb, t);
  _eu.setFromQuaternion(_qa);
  return [_eu.x, _eu.y, _eu.z];
}
