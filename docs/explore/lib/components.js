...
export function createRod(points, radius = 2, material, closed = false) {
  ...
  const segments = Math.max(vectors.length * 14, 96);
  const geometry = new THREE.TubeGeometry(curve, segments, radius, 20, closed);
...
export function createRing(outerDiameter, wireThickness = 4, material) {
  ...
  const geometry = new THREE.TorusGeometry(centerRadius, tubeRadius, 32, 96);
  const mesh = new THREE.Mesh(geometry, material);
  return mesh;
}
...
// Create a block (box) with softly chamfered edges (reads as a machined part,
// not a hard CAD cube)
export function createBlock(width, height, depth, material) {
  const radius = Math.min(Math.min(width, height, depth) * 0.06, 4);
  const geometry = radius > 0.2
...
export function createHoop(diameter, rodRadius = 2, material, segments = 128) {
