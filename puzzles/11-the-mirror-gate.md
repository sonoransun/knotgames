...
The spiral test works at a glance, but there is a crossing-by-crossing version that leaves nothing to judgment. It uses the same right-hand rule that computes crossing signs in Puzzle 3, The Prisoner's Ring:

1. **Orient the knot.** Pick a direction of travel around the loop and imagine arrows along the wire pointing that way. Either direction works — reversing it flips both arrows at every crossing at once, and the turn from one arrow to the other keeps its sense, so every sign comes out the same.
2. **Pick any crossing.** Two arrows meet there: one on the strand on top, one on the strand passing underneath.
3. **Read the sign.** Rotate the top strand's arrow onto the under-strand's arrow, turning through less than 180 degrees. If the rotation is counterclockwise, the crossing is **+1**. If it is clockwise, the crossing is **-1**.

Walk this through on the left panel of the comparison figure above. Read the figure for what it is — a shorthand: each panel draws the trefoil's outline as one unbroken loop and reduces each crossing to a short bar (horizontal at the top crossing, tilted at the two lower ones) that flags where one strand rides over the other, without drawing the full weave. Orient the loop counterclockwise — the direction its CCW arrow already indicates — and complete the top crossing in your mind's eye. Two strand segments pass through it, and under counterclockwise travel both head leftward, in step with the flow across the top of the loop: one descends toward the lower-left lobe while the other climbs toward the upper left. In the weave this panel stands for, the descending strand is the one on top. Now apply the rule: swing the top arrow onto the under arrow — from leftward-and-down to leftward-and-up, the short way sweeps clockwise, rising through due left. Clockwise means **-1**. The trefoil's three-fold symmetry repeats the same local picture at the lower-left and lower-right crossings, rotated by a third of a turn, so each of them also reads **-1**. Three crossings, all **-1**: left-handed, exactly as the panel is labeled. Mirror the walk-through onto the right panel's weave and every swing reverses — all three crossings read **+1**: right-handed.

In practice you never need all three. In the trefoil's standard three-crossing form, every crossing carries the same sign, so one honest sign check at a single crossing settles the handedness of the whole frame: **+1** says right-handed, **-1** says left-handed.
